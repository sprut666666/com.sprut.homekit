...
//  console.log('device: '+ JSON.stringify(device));
  ...
  if ('onoff' in device.capabilities)
  {
    ...
    {
      // Switch the capability on user input
      var dimVariable = HAS.predefined.Brightness(characteristicNum++, device.state.dim * 100 || 1, (value, callback) =>
      {
        // Calling the api
  ...
  if ('windowcoverings_state' in device.capabilities && 'dim' in device.capabilities == false)
  {
    ...
    var dimVariable = HAS.predefined.TargetPosition(characteristicNum++, stateCover2proc(device.state.windowcoverings_state), (value, callback) =>
    {
      // Calling the api
  ...
  }else if (device.class == 'windowcoverings' && 'dim' in device.capabilities)
  {
    //var LockCurrentState = HAS.predefined.LockCurrentState(characteristicNum++, state2num(device.state.locked));
    //capabilities.push(LockCurrentState);
    var currentPosition = HAS.predefined.CurrentPosition(characteristicNum++, device.state.dim*100, (value, callback) =>
    {
      // Calling the api
      console.log('1: ' + value);
      callback(HAS.statusCodes.OK);
    ...
    var dimVariable = HAS.predefined.TargetPosition(characteristicNum++, device.state.dim*100, (value, callback) =>
    {
      // Calling the api
      device.setCapabilityValue("dim", value/100) ;
  ...
  if ('thermostat_mode' in device.capabilities)
  {
    ...
    var targetHeatingCoolingState = HAS.predefined.TargetHeatingCoolingState(characteristicNum++,thermostatMode2targetHeatingCoolingState(device.state.thermostat_mode) , (value, callback) =>
    {
      // Calling the api
      if (value == 1)
      {
    ...
    var currentHeatingCoolingState = HAS.predefined.CurrentHeatingCoolingState(characteristicNum++, thermostatMode2currentHeatingCoolingState(device.state.thermostat_mode), (value, callback) =>
    {
      // Calling the api
  ...
  if ('target_temperature' in device.capabilities)
  {
    ...
    {
      var currentHeatingCoolingState = HAS.predefined.CurrentHeatingCoolingState(characteristicNum++, targeTemperature2currentThermostatMode(device.state.target_temperature), (value, callback) =>
      {
        // Calling the api
        console.log('currentHeatingCoolingState: ' + value);
        callback(HAS.statusCodes.OK);
      ...
      var targetHeatingCoolingState = HAS.predefined.TargetHeatingCoolingState(characteristicNum++, targeTemperature2thermostatMode(device.state.target_temperature), (value, callback) =>
      {
    ...
    var targetTemperature = HAS.predefined.TargetTemperature(characteristicNum++, targetTemperature2correct(device.state.target_temperature), (value, callback) =>
    {
      // Calling the api
      if (value > 28)
      {
  ...
  {
    var mute = HAS.predefined.Mute(characteristicNum++, device.state.volume_mute, (value, callback) =>
    {
  ...
  {
    // Switch the capability on user input
    var volume = HAS.predefined.Volume(characteristicNum++, device.state.volume_set * 100 || 1, (value, callback) =>
    {
      // Calling the api
      console.log('Switching dim for: ' + device.name + '. Value: ' +  value/100, "info");
    ...
    {
      var mute = HAS.predefined.Mute(characteristicNum++, device.state.volume_set == 0, (value, callback) =>
      {
        // Calling the api
        if (value)
        {
