...
function createDevice(device, id)
{
  ...
  {
    var airQuality = HAS.predefined.AirQuality(characteristicNum++, aqi2AirQuality(device.state.measure_aqi) || 0);
  ...
  device.on('$state', state =>
  {
    ...
    if ('measure_aqi' in device.capabilities)
    {
      let variable = aqi2AirQuality(state.measure_aqi);
