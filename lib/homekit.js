...
function createDevice(device, id)
{
  ...
  {
    var statusLowBattery = HAS.predefined.StatusLowBattery(characteristicNum++, state2num(device.state.alarm_battery), (value, callback) =>
    {
      callback(HAS.statusCodes.OK);
    });
  ...
  if ('measure_battery' in device.capabilities)
  {
    ...
    {
      var statusLowBattery = HAS.predefined.StatusLowBattery(characteristicNum++, alarmBattery(device.state.measure_battery), (value, callback) =>
      {
        callback(HAS.statusCodes.OK);
      });
      capabilities.push(statusLowBattery);
  ...
  device.on('$state', state =>
  {
    ...
    if ('volume_set' in device.capabilities)
    {
      let variable = state.volume_set * 100;
